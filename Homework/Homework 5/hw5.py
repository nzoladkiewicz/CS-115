...
def fast_change(amount, coins):
    ...
    def helperFunction(amount, coins, memo):
        ...
        if amount == 0:
            ...
            result = helperFunction(amount, coins[1:], memo)
        else:
            use_it = 1 + helperFunction(amount - coins[0], coins, memo)
            lose_it = helperFunction(amount, coins[1:], memo)
            result = min(use_it, lose_it)
        memo[(amount, coins)] = result
        return result
    return helperFunction(amount, tuple(coins), {})
